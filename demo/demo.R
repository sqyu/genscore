...
est <- estimate(x, setting="trun_gaussian", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=Inf, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=T)
...
readline("Press any key to continue to the next example.")
...
h_hp <- get_h_hp(mode=h_mode, para=h_param1, para2=h_param2)
h_hp$h # h(x)=min(x,3)
h_hp$hp # h'(x)=(x<=3)
# Sufficient statistics, named "elts"
elts_P <- get_elts(h_hp$h, h_hp$hp, x, setting="trun_gaussian", centered=F, profiled_if_noncenter=Inf, scale="norm", diagonal_multiplier=dm, use_C=TRUE, tol=tol) # P stands for Profiled
# Estimate using estimate() with elts_P
est2 <- estimate(x, setting="trun_gaussian", elts=elts_P, centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=Inf, h=h_hp$h, hp=h_hp$hp, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=T)
...
est_sub <- estimate(x, setting="trun_gaussian", centered=F, symmetric="symmetric", scale="norm", lambda1s = lambda1, lambda_ratio=Inf, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=T)
# Check if the result matches the previous one
...
for (i in 1:3){
  set.seed(i)
  x <- rtmvnorm(n, mean = mu, sigma = solve(K), lower = rep(0, p), upper = rep(Inf, p), algorithm = "gibbs", burn.in.samples = 100, thinning = 10)
  est <- estimate(x, setting="trun_gaussian", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=Inf, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=F, diagonal_multiplier=dm, return_raw=F)
...
## Exponential Square-Root Graphical Models
...
# with a=b=0.5.
x <- rab_arms_R(n, a=0.5, b=0.5, eta=mu, K=K, seed=2, burn_in=1000, thinning=1000, verbose=FALSE)
## Estimation
...
h_param1 <- 1.5
h_param2 <- 3
est_exp <- estimate(x, setting="exp", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=2, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=F)
...
mu[mu <= -1] <- abs(mu[mu <= -1])
x <- rab_arms_R(n, a=0.5, b=0, eta=mu, K=K, seed=3, burn_in=1000, thinning=1000, verbose=FALSE)
h_mode <- "min_pow"
h_param1 <- 1.5
h_param2 <- 3
est_gamma <- estimate(x, setting="gamma", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=2, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=F)
...
## General a-b Graphical Models
# Now choose a=1.5, b=0.5, and estimate using h(x)=\min(x,3)^0.5
# and `lambda_ratio=2`.
x <- rab_arms_R(n, a=1.5, b=0.5, eta=mu, K=K, seed=4, burn_in=1000, thinning=1000, verbose=FALSE)
h_mode <- "min_pow"
h_param1 <- 0.5
h_param2 <- 3
est_ab <- estimate(x, setting="ab_1.5_0.5", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=2, mode=h_mode, param1=h_param1, param2=h_param2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=F)
...
## (Untruncated) Gaussian Graphical Models
...
# version of the original score matching from Hyvärinen (2005);
...
est_gauss <- estimate(x, setting="gaussian", centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=T)
# Alternatively, get the summary statistics first (without specifying h and hp)
elts_gauss <- get_elts(NULL, NULL, x, setting="gaussian", centered=F, profiled_if_noncenter=F, scale="norm", diagonal_multiplier=dm, use_C=TRUE, tol=tol)
# Then pass it to estimate()
est_gauss2 <- estimate(x, setting="gaussian", elts=elts_gauss, centered=F, symmetric="symmetric", scale="norm", lambda_length=nlambda, lambda_ratio=2, verbose=F, tol=tol, maxit=maxit, BIC_refit=T, diagonal_multiplier=dm,  return_raw=T)
...
print(varhat(mu, sigma^2, mode, param1, param2, est_mu=T))
# The asymptotic variance for sigmasq assuming mu known
print(varhat(mu, sigma^2, mode, param1, param2, est_mu=F))
...
sdmu <- sqrt(varhat(muhat, sigma^2, mode, param1, param2, est_mu=T)/n)
...
sdsigmasq <- sqrt(varhat(mu, sigmasqhat, mode, param1, param2, est_mu=F)/n)
...
ses_mu <- sapply(muhats, function(muhat){sqrt(varhat(muhat, sigma^2, mode, param1, param2, est_mu=T)/n)})
...
ses_sigmasq <- sapply(sigmasqhats, function(sigmasqhat){sqrt(varhat(mu, sigmasqhat^2, mode, param1, param2, est_mu=F)/n)})
...
# variance, as well as the Cramér-Rao lower bound.
trials <- 100
...
# The Cramér-Rao lower bound on the variance of unbiased estimators of mu
...
# The Cramér-Rao lower bound on the variance of unbiased estimators of sigmasq
...
# (relative to the Cramér-Rao lower bound) of our estimators with
...
for (mu_i in 1:length(mus)){
  for (hi in 1:length(h_names))
    curves[hi,mu_i] <- varhat(mus[mu_i],sigma^2,modes[hi],param1s[hi],param2s[hi],est_mu=T)
...
for (sigma_i in 1:length(sigmas)){
  for (hi in 1:length(h_names))
    curves2[hi,sigma_i] <- varhat(mu,sigmas[sigma_i]^2,modes[hi],param1s[hi],param2s[hi],est_mu=F)
